...
void Deletely(std::vector<Text>& vec, Text& vectors)
{
	...
	std::cin >> id;
	std::cout << Blank(3) << "      <------------------------------------------------------>" << std::endl;
	std::cout << Blank(4) << std::left << std::setw(24) << "姓名" << std::setw(24) << "编号"
		<< std::setw(24) << "工资" << std::endl << std::endl;
	int i = 0;
	for (auto& message : vec)
	{
		if (message.id == id)
...
void Sortly(std::vector<Text>& vec, Text& vectors)
{
	...
	if (vec.size() == 0)
	{
		std::cout << std::endl << std::endl << Blank(6) << "    暂无数据！" << std::endl;
		return;
	}
...
void View(std::vector<Text>& vec, Text& vectors)
{
	...
	for (auto& v : vec)
	{
		std::cout << Blank(4) << std::left << std::setw(24) << v.name << std::setw(24) << v.id
			<< std::setw(24) << v.wages << std::endl;
	}
	std::cout << std::endl;
}
